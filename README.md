...
- **Explicit project modes** - each project selects a PR, validated main, or local delivery route, with an optional `+yolo` autonomy flag.
  Direct checks and one fresh-context review are the ordinary validation path; no-mistakes remains an explicit task opt-in.
- **Peer homes** - run a separate first mate per game or product against the same canonical repository: `bin/fm-home-seed.sh <id> <home> --peer <project>` provisions a home holding its own backlog and state plus the shared canonical project binding, and `bin/fm-open.sh <home> <harness>` opens it against this same code root, so every home runs one version, nothing can drift, and `/updatefirstmate` reaches them all at once.
...
Codex and OpenCode are also verified and supported as primary harnesses; Codex uses a Stop-launched detached supervisor with `codex queue` delivery, and OpenCode uses a TUI plugin, so both carry more harness-specific supervision tradeoffs than the three co-primaries.
...
# registers the canonical checkout and spawns two isolated workers in the active backend.
# Minutes later:
...
```
 ...
 ┌─────────────────────────────────────┐
 │ firstmate            (this repo)    │
 │ resolves canonical repos + routes  │
...
- [docs/verification/validation-pipeline.md](docs/verification/validation-pipeline.md) - active maintainer verification for how firstmate reads no-mistakes pipeline state, including the CI-ready return point on repositories that register no checks.
