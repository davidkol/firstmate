...
- Changes to how firstmate reads the no-mistakes pipeline (the `AGENTS.md` validation and PR-ready contracts, the generated brief in `bin/fm-brief.sh`, and run-step mapping in `bin/fm-crew-state.sh`) keep active empirical evidence in [`docs/verification/validation-pipeline.md`](docs/verification/validation-pipeline.md), verified against the installed binary rather than written from its documentation alone.
