...
repair_line() {
  ...
  case "$HARNESS" in
    claude)
      ...
      ;;
    codex)
      printf '%s%s\n' "$prefix" 'watcher supervision needs Stop-owned automatic recovery; inspect the hook registration in .codex/hooks.json and the watcher startup status before ending the turn.'
      ;;
    pi|pi-signed)
...
ordinary_wake_line() {
  case "$HARNESS" in
    claude)
      ...
      ;;
    codex)
      printf '%s\n' '- Ordinary wake: the Stop-owned background wake (bin/fm-codex-stop-autoarm.sh) already owns watcher continuity; drain and handle the wake, and do not arm another cycle yourself.'
      ;;
    pi|pi-signed)
