...
#
#   --supervise <session-id> (detached, internal)
...
#   - Delivery: `codex queue` must exist before arming, because a cycle whose
#     wake cannot be delivered is worse than no cycle.
#
...
Usage: fm-codex-stop-autoarm.sh                      Codex Stop hook; payload on stdin
       fm-codex-stop-autoarm.sh --supervise <id>     detached supervisor (internal)
...
binding_field() {  # <field>
  sed -n "s/^$1=//p" "$BINDING" 2>/dev/null | head -1
}

# True when $BINDING names a supervisor process that is still the same process
# it recorded. Identity, not just the pid, so a recycled pid is never signalled.
binding_owner_live() {
  local pid identity current
  pid=$(binding_field pid)
  case "$pid" in
    ''|*[!0-9]*) return 1 ;;
  esac
  fm_pid_alive "$pid" || return 1
  identity=$(binding_field identity)
  [ -n "$identity" ] || return 1
...
main_hook() {
  ...
  if binding_owner_live; then
    [ "$(binding_field session)" = "$session_id" ] && exit 0
    ...
    kill -TERM "$FM_CODEX_BINDING_PID" 2>/dev/null || true
    local i=0
    while [ "$i" -lt "$RETIRE_WAIT" ] && fm_pid_alive "$FM_CODEX_BINDING_PID"; do
      sleep 0.1
      i=$((i + 1))
    done
    fm_pid_alive "$FM_CODEX_BINDING_PID" && exit 0
  fi
...
queue_wake() {  # <session-id> <body>
  local session=$1 body=$2 encoded
  encoded=$(printf '%s' "$body" | "$SCRIPT_DIR/fm-operational-input.sh" encode watcher 2>/dev/null) || return 1
  [ -n "$encoded" ] || return 1
  "$CODEX_BIN" queue --thread "$session" --message "$encoded" >/dev/null 2>&1
}
