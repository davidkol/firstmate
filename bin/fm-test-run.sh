...
family_for_basename() {
  case "$1" in
    ...
    fm-learning-promote.test.sh|fm-lint.test.sh|\
    ...
    fm-validate.test.sh|fm-test-run.test.sh|fm-test-isolation-proof.test.sh)
      printf '%s\n' pure-contract-unit
      ;;
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-brief.sh|bin/fm-ensure-agents-md.sh|bin/fm-crew-state.sh|bin/fm-learning-promote.sh|\
