...
family_for_basename() {
  case "$1" in
    ...
    fm-review-convergence.test.sh|fm-validate.test.sh|fm-test-run.test.sh|fm-test-isolation-proof.test.sh)
      printf '%s\n' pure-contract-unit
      ;;
    fm-daemon.test.sh|fm-guard-stale-banner.test.sh|fm-pi-watch-extension.test.sh|\
    fm-session-lock-ancestry.test.sh|\
    fm-supervision-events.test.sh|fm-turnend-guard.test.sh|fm-wake-daemon-lifecycle-e2e.test.sh|\
    fm-wake-drain-open-decisions.test.sh|fm-wake-drain-open-decisions-cursor.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-primary-scope-lib.sh|bin/fm-project-mode.sh|bin/fm-promote.sh|bin/fm-review-convergence.sh|bin/fm-validate.sh|\
