...
# arm layer (bin/fm-watch-arm.sh and the Claude and Codex Stop auto-arms) needs exactly
...
#   autoarm     Claude and Codex Stop-hook auto-arm: the watcher is armed at each
#               turn end and exits on its wake, so it runs only BETWEEN turns.
#               Mid-turn a fresh beacon with no live watcher process is the
#               healthy state.
...
# because the watcher runs between turns; only a stale beacon is a lapse.
