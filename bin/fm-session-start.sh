...
# memory is stable session to session and is recoverable with one targeted read;
# live fleet identity - which tasks exist, their windows, worktrees, backends,
# and endpoint liveness - changes every session and is exactly what recovery
# depends on. So fleet state goes first and the memory files absorb the
...
#             projection cleanup and bootstrap's five mutating sweeps (legacy
#             PR-check migration, fleet sync, the local secondmate fast-forward
#             sweep, the secondmate liveness sweep, and X-mode artifact writes)
#             - and re-emit the rest. The wake-queue drain is NOT skipped:
#             queued records are this turn's work queue, they arrived after
#             startup, and a session that owns the lock is exactly the session
#             that must take them. Lock acquisition still runs, because
#             ownership must be re-verified rather than assumed: fm-lock.sh
#             already treats a lock this session's own harness holds as its own,
#             so the re-emit proceeds, while a lock another live session took
...
# repo built-in defaults, projects.md absent = project dispatch unavailable, etc. -
...
# and every other line it prints (its count) passes through untouched. Whatever
# is cut is disclosed exactly.
...
if [ "$REEMIT" -eq 1 ]; then
  ...
  printf 'secondmate convergence and liveness, PR-check migration, X-mode artifact writes,\n'
  printf 'and stale Herdr child cleanup - are NOT repeated.\n'
...
if [ "$READ_ONLY" -eq 1 ]; then
  ...
  DRAIN_OUT=$("$SCRIPT_DIR/fm-wake-drain.sh" --open-decisions all 2>&1)
  if [ -n "$DRAIN_OUT" ]; then
    printf '%s\n' "$DRAIN_OUT"
  else
