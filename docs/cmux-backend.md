...
The home label is the active home's readable prefix plus a stable short hash of the resolved Firstmate root; [`bin/fm-backend-hometag-lib.sh`](../bin/fm-backend-hometag-lib.sh) owns that derivation and [`configuration.md`](configuration.md#home-kinds) owns the home kinds it distinguishes.
