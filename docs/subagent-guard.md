...
This is the same predicate the native session-open adapters (`bin/fm-sessionstart-run.sh` and `bin/fm-sessionstart-nudge.sh`) and `bin/fm-turnend-guard.sh` use, so the tracked primary-scoped hooks cannot drift apart.
