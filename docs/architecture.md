...
`bin/fm-validate.sh` derives every mode's skip set, never skips review, requires an executed final-change capture from the task worktree, publishes it for isolated pipeline rounds, and replaces caller paraphrases with the brief's canonical selected-review intent; [`verification/validation-pipeline.md`](verification/validation-pipeline.md#the-review-step-runs-alone-and-in-its-own-agent-process) owns the empirical evidence that the review step runs alone in a separate agent process.
...
Durable project-intrinsic agent knowledge lives in each project's committed `AGENTS.md`, with `CLAUDE.md` as a regular file that imports it with `@AGENTS.md`.
...
It refuses a case-variant real memory file such as a lowercase `agents.md`, because the uppercase `@AGENTS.md` import would not resolve portably on a case-sensitive filesystem, and surfaces the mismatch for manual reconciliation.
