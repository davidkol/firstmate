...
The shared classifier in `bin/fm-classify-lib.sh` owns status routing: worker intent distinguishes coordination from routine conversation.
The watcher consumes every unseen status append before advancing its existing signal signature, so a later progress note cannot hide an explicit request or outcome.
Its grace scan coalesces each changed file once and queues only eligible files.
...
An owned-run failure attributed to the crew's own code is announced once for both delivery processes, across changed panes and watcher restarts, because what is consumed is the failed run's exact producer run ID (`bin/fm-crew-state.sh --with-run-identity`) rather than a delivered flag; a later run that fails is a different ID and delivers even though an idle worker's unchanged pane gave the watcher no chance to observe the rerun in between.
The ID alone is the key, so consumption is unaffected by which surface answered or whether that answer projected the head short or full: the reader takes the ID from `axi status` when it attributed a run there, and otherwise asks the running daemon's existing read-only `get_run` and `get_runs_for_head` methods for the exact repository, branch and full head, validating every returned record before naming a run.
A query that fails, times out, answers with something unusable, or attributes a failed run it cannot name never re-arms a consumed failure, and equally never announces or consumes one: not knowing is neither evidence that a failed run recovered nor a second failure, so delivery for that observation stays pending and the next bounded observation asks again.
Only a known nonfailed state with `run-step` provenance can retire a delivery record; an unknown run state or a fallback to the worker's status log or semantic busy pane cannot establish recovery.
While the run ID is unreadable - no daemon socket, no `python3`, an answer with no run to seed the lookup, or a reply past the frame bound - the automatic observer announces no owned-run failure and cannot promise a complete history of that window.
A worker's own explicit `failed:`, `blocked:` or `needs-decision:` status still routes through the independent status-signal path.
Every repeated authoritative read of an already-classified pane goes through one `FM_STALE_ESCALATE_SECS` cadence, including retained pause tracking and unavailable failure identity; each attempt consumes its interval even when the reader supplies no usable answer.
That observation precedes status-shape suppression, so a run that fails after its worker went idle is reported whether its log carries a pre-validation outcome, a routine progress note or a declared wait.
A consumed failure then keeps that idle pane quiet rather than re-surfacing it as a possible wedge, and the pane-based stuck timer returns as soon as the worker positively resumes: a completed turn recorded after the failure was consumed, which is what a worker fixing that same run locally at the same head produces, or a new run reported working.
A re-rendered idle pane and repeated routine notes are not resumption and never restart that timer, so a resumed worker that then stalls still reaches its ordinary wedge escalation with its accumulated count, while a genuinely idle consumed failure stays silent.
The reader reaches that failure in the first place because pipeline-owned custody is bound on the producer's own fields: it accepts a terminal failed or cancelled run whose commits are still unpublished, and it treats the abbreviated head `axi status` prints and the full `branch_sync` head as one commit when the shorter is a real prefix of the longer, without ever fetching the gate's objects into the worker.
...
When a surfaced turn-end or status signal and the first static-pane observation describe the same boundary, the watcher absorbs that one bare stale duplicate into the normal stuck timer.
When a matching active run owns the work, a stable token over its run identity, head, active step, and bounded step-log tail resets that timer only when the owned pipeline evidence changes, while an unchanged token still wedge-escalates.
...
A declared external wait retains one bounded internal recheck per pause window without producing another captain wake.
...
`fm-wake-drain.sh` builds its fleet-wide OPEN DECISIONS section through `fm-classify-lib.sh`'s cursor-backed incremental scan using the authoritative `status_open_decisions` fold semantics, so each drain reads only new status-log appends and a buried decision remains durable until explicit resolution.
Session recovery requests the complete open set, while routine drains print only newly opened, reopened, or materially changed decision records and remain silent for decisions already surfaced unchanged.
The explicit resolution is written by the actor that answers, not the busy worker: `fm-send`'s `--resolve-key` appends the closing `resolved` line to this home's own copy of the ledger at answer time, which covers crewmates and local secondmates identically because only the answer message itself crosses the backend transport.
...
During an active daemon-owned fix round, explicit `branch_sync.state: pipeline_owned` provenance also preserves attribution when its local and submitted heads equal the worktree tip and its pipeline run and current head equal the active run, even when the pipeline head has diverged.
...
That block owns the live wait shape for the running primary harness: Claude's Stop `asyncRewake` hook owns tokenless re-arm cycles, Grok uses background-notify cycles, Codex uses a Stop-launched detached supervisor that wakes the bound conversation with `codex queue`, Pi and pi-signed use the same two tracked primary extensions, and OpenCode uses its TUI plugin.
For Codex, no supervisor is launched when the home has no supervision need, the parked wait renders no foreground call and spends no model tokens, and an actionable close returns as one marked operational input that the session drains.
`codex queue --thread` is the supported runtime primitive that both resumes the bound conversation and leaves it responsive to captain messages while it waits, verified against the exact client recorded in [`verification/supervision.md`](verification/supervision.md), so a Codex primary no longer renders a bounded foreground call per live supervision interval.
`bin/fm-watch-checkpoint.sh` survives only as the short manual recovery probe for an inspection the turn-end guard demanded.
...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, if work or X-mode relay polling has an unhealthy model-aware supervision verdict, or if queued wakes are waiting to be drained.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when work or X-mode relay polling needs supervision and no identity-matched watcher lock with a fresh beacon is live, direct Stop hooks block and passive turn-end hooks force one bounded follow-up.
...
Herdr's durable default container shape is workspace-per-home plus tab-per-task: the primary home uses workspace label `firstmate`, peer homes use `peer-<peer-id>`, secondmate homes use `2ndmate-<secondmate-id>`, and recovery/list-live scopes to the current `FM_HOME`'s workspace.
...
The same trusted default-branch copy supplies `commands` and `agent` while `allow_repo_commands` is unset, so a branch editing `commands.test` still validates under the default branch's command; [`verification/validation-pipeline.md`](verification/validation-pipeline.md#repository-commands-come-from-the-trusted-default-branch-config) owns that active empirical evidence.
...
Ship tasks change projects and ship by project mode (`no-mistakes`, `validated-main`, `direct-PR`, or `local-only`); scout tasks leave standalone investigation reports at `data/<id>/report.md` and never push.
...
`local-only` projects stay with the main first mate because they merge into the main local checkout instead of a remote-backed path.
...
Once those refusals pass, and before any destructive step, teardown concludes the task's own validation run when it is still parked at a gate and reaps processes leaked under that task's own worktree or tasktmp, so removing the worker never strands a run holding a fleet slot or a background process holding the worktree; `bin/fm-nm-run-lib.sh` is the single owner of the branch-and-code-identity rule that attributes such a run to a worktree, shared with `bin/fm-crew-state.sh` so both bind runs by the same contract.
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the landed-work proofs, PR-discovery fallback, stale-lock recovery procedure, and both pre-teardown cleanup steps.
...
The locked session-start bootstrap step, teardown of a remote-backed ship task, and merged-PR wake handling refresh remote-backed project clones when the clone is safe to move.
