...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that stop making owned progress past `FM_STALE_ESCALATE_SECS`, and heartbeat backstop hits.
...
A crew that declares `paused:` for a known external wait is separately absorbed while idle and rechecked internally on the longer pause cadence, rather than being treated as a possible wedge or producing repeated captain interruptions.
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake, then applies that silent cadence to an unchanged `paused:` or durable `captain-held` endpoint only when the backend confidently reports its agent dead.
...
For Codex, quiet checkpoint expiry returns control without output or a routine drain, a preexisting durable queue returns an immediate `queue:` reason, and no checkpoint runs when the home has no supervision need.
Codex still renders the one bounded foreground call required for each live supervision interval because Firstmate has no supported runtime primitive that both wakes Codex immediately and preserves user-message responsiveness without that call.
...
`bin/fm-validate.sh` starts and drives every mode's run, derives the skip set, never skips review, requires an executed final-change capture from the task worktree, publishes it for isolated pipeline rounds, and replaces caller paraphrases with the brief's canonical selected-review intent.
`bin/fm-review-convergence.sh` owns the executable review budget and durable evidence state: one initial review, one batched remediation, at most one bounded closure review, then tests or review-only completion, with another remediation, scope expansion, stale proof, and disproportionate test amplification refused before they can reopen the audit.
[`verification/validation-pipeline.md`](verification/validation-pipeline.md#review-convergence-is-executable-and-handoff-safe) owns the causal fixture evidence for that cutover, while its [review process evidence](verification/validation-pipeline.md#the-review-step-runs-alone-and-in-its-own-agent-process) shows that the initial review still runs in a separate agent process.
...
Under `validated-main` that crew branch is fast-forwarded into the default branch, so those evidence commits land with the change.
