...
A query that fails, times out, answers with something unusable, or attributes a failed run it cannot name never re-arms a consumed failure: not knowing is not evidence that a failed run recovered, so the receipt is held as it stands and silently adopts the ID once a later observation supplies it.
...
A consumed failure then keeps that idle pane quiet rather than re-surfacing it as a possible wedge; the pane-based stuck timer is restored as soon as the crew is positively read as working again, including on the same pane it failed on.
