...
KNOWN LIMIT, stated rather than designed around: while the run ID is unreadable - no daemon socket, no `python3`, an answer with no run to seed the lookup, or a reply past the frame bound - the automatic observer announces no owned-run failure at all and cannot promise a complete history of that window. It is an availability limit; a worker's own explicit `failed:`, `blocked:` or `needs-decision:` status still routes through the status-signal path, which does not consult the producer.
A pane hash this loop already classified is re-observed on one shared bounded cadence, ahead of every status-shape suppression rather than per shape, so a run that fails after its worker went idle is reported whether its log carries a pre-validation outcome, a routine progress note or a declared wait, and the reader is called at most once per `FM_STALE_ESCALATE_SECS` per window whether or not it answers.
