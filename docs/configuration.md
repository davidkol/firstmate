...
For explicitly opted-in tasks, the tracked `.no-mistakes.yaml` keeps test evidence outside the repo and pins `commands.lint` to `bin/fm-lint.sh` so local lint matches CI.
...
It sets `commands.test` to a scoped `bin/fm-test-run.sh --changed --require-nonempty` selection rather than a complete `tests/*.test.sh` walk.
...
The budget is an on-demand curation report, not a session-start gate: bootstrap only publishes the setting, and an over-budget total never blocks or slows a session start.
...
Secondmate routes cover remote-backed projects (`no-mistakes`, `validated-main`, and `direct-PR`); `local-only` projects remain main-firstmate work.
...
An omitted mode defaults to `direct-PR`; explicit existing modes remain unchanged.
`AGENTS.md` section 7 owns direct validation and per-task no-mistakes opt-in, independently of these delivery labels.
...
This section owns the operator contract for primary Codex launch policy and its away-mode boundary; executable mechanics remain in the referenced script headers and help.
Primary Codex launch is separate from worker dispatch: [`bin/fm-codex-primary.sh`](../bin/fm-codex-primary.sh) owns the supported fresh and resumed primary entry, supplies `approval_policy="never"` plus `sandbox_mode="danger-full-access"`, explicitly enables hooks, and accepts work only after its `codex doctor --json` report proves `Never`, `unrestricted`, enabled network access, and enabled hook support.
It refuses Codex non-interactive, service, maintenance, and fork subcommands because their tracked-hook lifecycle is not the interactive fresh/resume lifecycle that this entry proves.
It anchors both the policy probe and the TUI to this tracked code root and refuses caller root or remote-server overrides because their hook and runtime policy would not be the state it proved.
It also refuses caller config, profile, and feature layers because Codex accepts multiple TOML spellings and later CLI config can replace the verified policy or disable the required hooks; ordinary non-config flags such as `--model` remain available.
The launcher also loads Firstmate's tracked Codex hooks without relying on persisted project trust; the tracked source and Firstmate's authority rules remain the safety boundary for destructive, irreversible, external, and security-sensitive actions.
[`bin/fm-open.sh`](../bin/fm-open.sh) routes a peer home's bare `codex` harness name through that launcher, while a raw Codex command is not a supported primary entry.
Codex workers remain owned by `bin/fm-spawn.sh` and retain their established `--dangerously-bypass-approvals-and-sandbox` launch path without the primary policy probe.
The two away lifecycle entries call [`bin/fm-codex-away-pretool-check.sh`](../bin/fm-codex-away-pretool-check.sh) before process-identity work and refuse when Codex marks the command with its restricted macOS sandbox environment or when the OpenAI-signed live Codex ancestor lacks both the launcher's immutable policy argv and an open rollout whose latest effective turn context is `never`/`danger-full-access`.
The entry check deliberately ignores caller-controlled `CODEX_HOME` and verification markers, uses fixed system process tools, verifies the executable's Developer ID and team, reads the process's NUL-delimited arguments from the macOS kernel, requires the exact launcher policy prefix with no later policy layer, and fails closed when no held-open rollout resolves.
When an authenticated Codex process holds multiple rollouts for its internal threads, every latest effective turn context must be `never`/`danger-full-access`; caller-controlled `CODEX_THREAD_ID` cannot select around a restricted one.
The same executable remains registered as a Codex PreToolUse defense for native Bash calls, but the lifecycle call is authoritative because Codex 0.147.0 code-mode shell execution does not pass through that native-tool hook.
An active away session created by the supported primary path remains under approval policy `Never`, so a later command failure returns to the model instead of creating an interactive approval dialog; raw Codex is outside the supported primary boundary and cannot pass the away declaration gate merely because its filesystem sandbox is unrestricted.
Enabled primary-session turn-end guard integrations are tracked as repo-level hook files and documented in [`docs/turnend-guard.md`](turnend-guard.md).
Kimi remains outside the primary turn-end guard integrations; [`docs/turnend-guard.md`](turnend-guard.md#compatibility-limits) owns its separate captain-approved crew wake hook.
Primary-session watcher wake protocols are rendered at session start by [`bin/fm-supervision-instructions.sh`](../bin/fm-supervision-instructions.sh) from [`docs/supervision-protocols/`](supervision-protocols/).
Claude's Stop `asyncRewake` hook owns tokenless re-arm cycles, Grok uses background-notify cycles, Codex uses a Stop-launched detached supervisor that wakes the bound conversation with `codex queue`, Pi and pi-signed use the same two tracked primary extensions, and OpenCode uses its TUI plugin.
...
No-mistakes is optional and only used when explicitly requested for a task; its absence or version never blocks ordinary dispatch.
...
It uses the same live secondmate discovery and propagation helper as bootstrap, prints each live home's `crew-dispatch.json`, `crew-harness`, `backlog-backend`, `herdr-presentation-spaces`, `startup-memory-budget`, and `data/captain-shared.md` result as `pushed`, `unchanged`, `skipped`, or `error`, and exits non-zero for real propagation errors or config-reread send failures.
...
FM_PROC_ROOT_OVERRIDE=   # alternate /proc root for the Linux process-identity reads in fm-wake-lib.sh and fm-teardown.sh's leaked-process reap, mainly for tests
...
FM_CODEX_WATCH_CHECKPOINT=180   # seconds per bounded foreground watcher checkpoint, the Codex manual recovery probe only
...
FM_CODEX_AUTOARM_RETIRE_WAIT=50   # deciseconds the Codex Stop hook waits for a supervisor bound to a different conversation to exit before declining to arm and opening a failure episode
FM_CODEX_PUBLISH_ATTEMPTS=3   # bounded `codex queue` publication attempts for one wake or failure notice; accepted values are 1 through 5. An unencodable body is permanent and never retried
FM_CODEX_PUBLISH_RETRY_DELAY=1   # seconds between those publication attempts
...
FM_WATCH_DELIVERY_MAX_BYTES=65536   # size cap for the watcher's terminal-delivery ledger, which lets an attached arm report a reason it never saw on stdout
...
FM_TEARDOWN_NM_TIMEOUT=10   # seconds allowed per no-mistakes query or abort inside fm-teardown.sh's pre-teardown parked-run conclusion; invalid values use 10
...
CLAUDE_CONFIG_DIR=      # optional Claude config home whose projects/ store fm-brief.sh reads to name a project's existing curated notes in a brief; defaults to ~/.claude
