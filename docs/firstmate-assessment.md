...
- **Made:** The isolated implementation branch now gives primary and peer homes one explicit canonical repository path, anchors Treehouse work to it, records and re-verifies task repository identity, ignores retained clones during fleet refresh, and preserves non-conflicting untracked files during guarded synchronization and landing.
  ...
  The change passed focused automated checks and a real Treehouse identity smoke, landed on Firstmate `main` as `e68553f`, was pushed to `origin/main`, and migrated the private Martyrdome registry to the `projects/Godot/Martyrdome` checkout under the captain's home directory with `validated-main` delivery.
...
- **Superseded - maintain momentum while he is away:** The captain said, "great, im going to bed, do as much as you can overnight," making continued progress during his absence a specific criterion for the 2026-08-15 continuation; the 2026-09-03 captain-in-the-loop decision explicitly replaced that implementation goal with the trial recorded in I1-C5.
...
- Firstmate verified commit `a5a8eda` only in the managed `projects/Martyrdome` clone under its `fm-homes/martyrdome` home.
...
- When the captain could see only `target-design.md`, Firstmate linked that managed clone's `docs/target-design` directory as though it were the captain's working folder.
