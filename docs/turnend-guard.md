...
`tests/fm-codex-stop-autoarm.test.sh` covers the Codex Stop-owned background wake's inert gates, immediate hook return with detached delivery, quiet idle, same-conversation reuse, stale-supervisor retirement and rebinding, mid-cycle away mode, one-notice failure episodes, arm-output cleanup on retirement, the published and both unpublished wake records read back by the real guard, and second-home isolation.
