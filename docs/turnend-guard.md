...
`tests/fm-codex-stop-autoarm.test.sh` covers the Codex Stop-owned background wake's inert gates, immediate hook return with detached delivery, quiet idle, same-conversation reuse, stale-supervisor retirement and rebinding, mid-cycle away mode, one-notice failure episodes, and second-home isolation.
