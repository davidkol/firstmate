...
The first is a live supervisor recorded in `state/.codex-autoarm-session` whose pid is alive and still matches its recorded process identity.
...
No other outcome is recovery evidence: `arming` from a dead process, `failed`, `afk`, `clean`, an unparsable or future-dated timestamp, and any aged record all still block.
...
`tests/fm-codex-stop-autoarm.test.sh` covers the Codex Stop-owned background wake's inert gates, immediate hook return with detached delivery, quiet idle, same-conversation reuse, stale-supervisor retirement and rebinding, mid-cycle away mode, one-notice failure episodes, arm-output cleanup on retirement, and second-home isolation.
