...
Codex uses the same Stop-owned model through `bin/fm-codex-stop-autoarm.sh`, which detaches its supervisor instead of holding the stop open and wakes the bound conversation with `codex queue`.
Grok retains its tracked background-task notification protocol.
No adapter starts a replacement with shell `&`.

The turn-end guard remains the final backstop rather than the normal continuity mechanism and cooperates with the auto-arm in its `--claude` and `--codex` modes.
...
Claude depends on the Stop `asyncRewake` rewake, Grok retains native background-completion notifications, and Codex depends on a detached Stop-launched supervisor plus `codex queue` delivery into the bound conversation.
