...
5. If the Stop hook does not claim the home or reports an exhausted failure, inspect its registration and watcher startup path before ending blind.
   ...
   A manual `bin/fm-watch-arm.sh` run during that inspection stays subject to the PreToolUse seatbelt (`bin/fm-arm-pretool-check.sh`) registered in `.claude/settings.json`, which denies shell `&`, a truncating pipe, or bundling.
