...
When implementation is separately authorized, promote the existing scout through `bin/fm-promote.sh` rather than creating a duplicate task, supplying the explicit tier, authoritative outcome, and only the conditional evidence that `delivery-doctrine` requires; the scout report remains evidence and never authorizes its own promotion.
