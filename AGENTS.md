...
Teardown also refuses when the task no longer holds the worktree pool slot its record names, because another task may be living there; explicit discard authority never extends to that refusal, since it authorizes discarding this task's work and never another task's.
A task's slot is reserved by the pool's own durable lease from spawn until teardown returns it under that lease, so no other task can be given the slot while cleanup is running; teardown refuses rather than returning a leased slot unguarded.
