...
# Both Stop-hook harnesses arm the watcher at turn end and let it exit on its
# wake, so both must resolve to the between-turns autoarm model. Codex used to
# have its own checkpoint model; a regression back to a codex-specific value
# would silently change what the pull guard treats as healthy mid-turn.
test_stop_hook_harnesses_share_the_autoarm_supervision_model() {
  local out
  out=$(FM_HOME="$TMP_ROOT" bash -c '
    . "$1/bin/fm-wake-lib.sh"
    for harness in claude codex opencode pi grok unknown; do
      printf "%s=%s\n" "$harness" "$(fm_supervision_model_for_harness "$harness")"
    done
  ' _ "$ROOT")
  assert_contains "$out" "claude=autoarm" "claude must resolve to the autoarm supervision model"
  assert_contains "$out" "codex=autoarm" "codex must resolve to the autoarm supervision model"
  assert_contains "$out" "opencode=persistent" "opencode must stay on the persistent supervision model"
  assert_contains "$out" "pi=persistent" "pi must stay on the persistent supervision model"
  assert_contains "$out" "grok=persistent" "grok must stay on the persistent supervision model"
  assert_contains "$out" "unknown=persistent" "an unknown harness must stay on the persistent supervision model"
  assert_not_contains "$out" "checkpoint" "no harness may resolve to the retired checkpoint supervision model"
  pass "fm-guard stale banner: claude and codex share the autoarm supervision model"
}
