...
SH
  ...
  # happens to be on the test runner's own PATH, which on this fleet is a
  # shared daemon no test may touch. Tests exercising the run-abort path
  # override FM_FAKE_AXI_STATUS/FM_FAKE_NM_ABORT_LOG before run_teardown.
...
# Teardown has two distinct lsof consumers, so this stub answers each on its own
# terms: the lock-holder query (`lsof -- <path>`) reports provably no holder,
# while the pre-teardown cwd sweep (`lsof -a -d cwd -Fpn`) succeeds with an
# empty result, which is the truth in these hermetic cases - nothing runs with a
# cwd under the task's worktree or tasktmp. Answering the sweep with lsof's
# not-found exit instead would look like a failed scan and refuse teardown,
# which is a different condition these lock cases are not about.
add_lsof_no_holder() {
  local case_dir=$1
...
# fakebin/lsof stub: the lock-holder query errors, so a lock is never judged
# stale and teardown must refuse without removing it.
# Scoped to that query on purpose. Teardown's other lsof consumer, the
# pre-teardown cwd sweep, refuses on its own when a scan cannot be established,
# and it runs first - so a stub that errored on every query would refuse there
# and the lock-staleness fail-safe below would never be reached, silently losing
# that coverage. The wholly-broken-lsof condition is exercised separately by
# test_lsof_error_refuses_before_removal, which installs its own stub.
add_lsof_error() {
  local case_dir=$1
  cat > "$case_dir/fakebin/lsof" <<'SH'
#!/usr/bin/env bash
...
test_teardown_prompts_tasks_axi_done_when_compatible() {
  ...
  printf '%s\n' 'pr=https://github.com/example/repo/pull/7' >> "$case_dir/state/task-x1.meta"
  mkdir -p "$case_dir/state/task-x1.review-convergence"
  : > "$case_dir/state/task-x1.review-convergence/manifest"
  add_compatible_tasks_axi "$case_dir"

  out=$(run_teardown "$case_dir") || fail "teardown failed with compatible tasks-axi"
  [ ! -e "$case_dir/state/task-x1.review-convergence" ] \
    || fail "teardown retained the task's review-convergence mirror"
