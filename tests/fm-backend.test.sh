...
# Every library a copied sibling or a BASE_REF-extracted script sources must be
# listed here too, or that script aborts under set -eu the moment the old bin/
# is exercised, and the conformance case fails for a missing file rather than a
...
OLD_BIN_UNCHANGED_SIBLINGS="fm-gate-refuse-lib.sh fm-guard.sh fm-lock-lib.sh fm-tasks-axi-lib.sh fm-pr-lib.sh fm-tangle-lib.sh fm-tmux-lib.sh fm-composer-lib.sh fm-wake-lib.sh fm-classify-lib.sh fm-doctrine-contract.sh fm-line-cap-lib.sh fm-busy-lib.sh fm-supervision-lib.sh fm-ff-lib.sh fm-config-inherit-lib.sh fm-startup-memory-budget-lib.sh fm-project-mode.sh fm-project-lib.sh fm-harness.sh fm-crew-state.sh fm-nm-run-lib.sh fm-decision-hold.sh fm-backend.sh fm-operational-input.sh"
...
test_captain_directed_dispatch_refresh_preserves_the_worker_prompt() {
  ...
  for tier in T1 T3; do
    ...
    assert_grep 'in this same warm context' "$brief" \
      "captain-directed $tier worker-facing prompt lost its warm-context fix loop"
