...
#   6. Flag misuse (--key, empty message, explicit backend target) refuses.
#
# Upstream's two remote-secondmate legs are deliberately absent: this fork
# carries no remote-secondmate transport (bin/fm-send.sh has no remote path),
# so there is no behavior here for them to assert.
set -u
