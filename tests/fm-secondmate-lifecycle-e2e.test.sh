...
#   - a legacy no-mistakes project is cloned without initializing validation tools
...
phase_seed() {
  ...
  # Seeding neither invokes optional tools nor alters existing gate state.
  assert_absent "$SUB/projects/gamma/.no-mistakes-init" "seed initialized an optional gate"
  assert_absent "$SUB/projects/gamma/.no-mistakes-doctor" "seed invoked optional doctor"
  ...
  pass "seed: registry scope+projects, charter copied, clones+origins, no optional gate initialization"
}
