...
When a request moves accepted game design toward implementation, route the first ship to the captain-directed builder scaffold from `bin/fm-brief.sh --captain-directed`; `game-development-process` owns its review and landing compatibility, and the ordinary route must not be substituted silently.
