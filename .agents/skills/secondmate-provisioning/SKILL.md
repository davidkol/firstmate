...
`config/startup-memory-budget` is additionally content-validated on the primary side because it is consumed as a local safety boundary: a malformed primary value is a propagation error rather than bytes to copy, while a malformed destination value is exactly what the primary's validated value overwrites, so a corrupted secondmate copy can still re-converge; `docs/configuration.md` "Startup memory budget" owns the accepted format.
