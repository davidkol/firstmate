...
- Any other `FLEET_SYNC:` line reached the digest through the unfiltered relay, which forwards the whole sync transcript instead of only the three classified outcomes.
  ...
  Repair with `bin/fm-project-path-set.sh <project-id> <absolute-repo-root>`, and read that script for the exact refusals it enforces before reading one of them as a failure of your own fix.
