...
- Any other `FLEET_SYNC:` line reached the digest through the unfiltered relay, which forwards the whole sync transcript instead of only the three classified outcomes.
  ...
  Repair with `bin/fm-project-path-set.sh <project-id> <absolute-repo-root>`; [`docs/configuration.md` "Canonical project repositories"](../../../docs/configuration.md#canonical-project-repositories) owns the refusals that repair enforces.
