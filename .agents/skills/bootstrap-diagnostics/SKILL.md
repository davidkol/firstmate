...
- Any other `FLEET_SYNC:` line reached the digest through the unfiltered relay, which fires only when the fleet refresh timed out or exited non-zero and then forwards the whole sync transcript instead of only the three classified outcomes, so any unclassified line is itself evidence that the refresh did not complete cleanly.
  A line that is neither `skipped`, `recovered`, nor `STUCK` can be routine progress such as `already current at <path>`, `synced <a>..<b> at <path>`, or `pruned <branch>`, or it can be a hard refusal, so read the line rather than pattern-matching it as sync noise.
  `PROJECT_PATH_INVALID` is the refusal that stops work outright: `fm_project_validate_root` rejected a registered repository path, and because `fm_project_resolve` validates every registry record and fails on the first bad one, no task can be dispatched to any project in this home until the registry is corrected.
  In a primary or peer home the usual cause is a canonical path registered under `<home>/projects`, which the registry refuses outside a secondmate home even though an inactive clone may still sit there, so register a checkout outside that directory instead.
  Repair with `bin/fm-project-path-set.sh <project-id> <absolute-repo-root>`; [`docs/configuration.md` "Canonical project repositories"](../../../docs/configuration.md#canonical-project-repositories) describes the registry contract that repair works within, but it does not list every refusal the repair can print, so read the printed refusal itself.
