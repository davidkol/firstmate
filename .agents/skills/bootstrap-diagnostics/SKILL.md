...
- Any other `FLEET_SYNC:` line reached the digest through the unfiltered relay, which forwards the whole sync transcript instead of only the three classified outcomes.
  ...
  In a primary or peer home the usual cause is a clone under `<home>/projects`, which `AGENTS.md` section 2 reserves for secondmate homes alone, so register a canonical checkout outside that directory instead.
  Repair with `bin/fm-project-path-set.sh <project-id> <absolute-repo-root>`, which needs the existing and the requested checkout present at the same time so it can compare their origins.
  It refuses while any task record under `state/<task-id>.meta` still names the old path, and it refuses again when the rewritten registry does not resolve, so a second invalid registry entry blocks the repair of the first.
