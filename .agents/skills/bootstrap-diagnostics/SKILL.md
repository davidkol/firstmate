...
- Any other `FLEET_SYNC:` line reached the digest through the unfiltered relay and is an unclassified refusal rather than a benign skip.
  A line that is neither `skipped`, `recovered`, nor `STUCK` must be read before dispatching to that project, because the shared prefix makes a hard blocker look like routine sync noise.
  `PROJECT_PATH_INVALID` is the case that stops work outright: `fm_project_validate_root` rejected the registered repository path, so `fm_project_resolve` fails and no task can be dispatched to that project from this home until the registry is corrected.
  ...
  Repair with `bin/fm-project-path-set.sh <id> <absolute-repo-root>`, which needs the existing and the requested checkout present at the same time so it can compare their origins, and which refuses while any `state/<id>.meta` still names the old path.
